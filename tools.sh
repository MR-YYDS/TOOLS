...
dd_xitong() {
		...
		dd_xitong_1() {
		  echo -e "重装后初始用户名: ${gl_huang}root${gl_bai}  初始密码: ${gl_huang}Gzh!!)517${gl_bai}  初始端口: ${gl_huang}22${gl_bai}"
		  echo -e "按任意键继续..."
		  read -n 1 -s -r -p ""
		  install wget
		  dd_xitong_MollyLau
		  ...
		  while true; do
			...
			echo "5. Ubuntu 24.04               6. Ubuntu 22.04"
			echo "7. Ubuntu 20.04               8. fnos飞牛公测版""
			echo "------------------------"
			echo "0. 返回上一级选单"
			echo "------------------------"
...
linux_Settings() {
	...
	while true; do
	  ...
	  echo -e "${gl_kjlan}26.  ${gl_bai}修复OpenSSH高危漏洞（岫源）"
